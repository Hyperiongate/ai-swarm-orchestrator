...
Last Updated: February 20, 2026 - MOVED to routes/ folder + FIXED db path

CHANGELOG:

- February 20, 2026: MOVED from root to routes/ folder
  REASON: app.py imports from routes.pattern_recognition. The file existed
  at the root as pattern_recognition.py, causing ImportError on every startup.
  Moving it to routes/pattern_recognition.py matches the import statement.

- February 20, 2026: FIXED hardcoded database path (same bug as task_analysis.py)
  BUG: sqlite3.connect('swarm_intelligence.db') uses a relative path, which
       resolves to the current working directory at runtime (ephemeral, resets
       on redeploy). All queries returned empty results.
  FIX: Import DATABASE from config and use that path as the default.
       DATABASE = '/mnt/project/swarm_intelligence.db' (persistent disk).
       KNOWLEDGE_DB_PATH env var still overrides if set.

  No other logic changed. All endpoints and response formats unchanged.
...
def get_pattern_insights():
    ...
    try:
        # ====================================================================
        # FIX (February 20, 2026): Use persistent DATABASE path from config
        # BEFORE: sqlite3.connect('swarm_intelligence.db')
        #         -> ephemeral local path, resets on redeploy, always empty
        # AFTER:  sqlite3.connect(db_path) where db_path = DATABASE
        #         -> '/mnt/project/swarm_intelligence.db' (persistent disk)
        # ====================================================================
