...
Version 14.0 - Benchmark Comparison Edition
...
and updates PowerPoint templates with survey data.

NEW in v14:
- Benchmark Comparison reports: Compare overall results to normative database
- Removed standalone "Create PowerPoint" (use template update instead)
"""
...
class SurveyCrosstabApp:
    ...
    def run(self):
        ...
        self.root = tk.Tk()
        self.root.title("Survey Crosstab Tool")
    ...
    def create_footer(self):
        ...
        version = tk.Label(
            footer,
            text="Survey Crosstab Tool v11.0 - Excel & PowerPoint Edition",
            font=self.fonts['small'],
            bg=self.COLORS['white'],
            fg=self.COLORS['gray_500']
        )
    ...
    def show_step_1(self):
        ...
        tk.Label(
            title_frame,
            text="Choose an existing project or start a new one",
            font=self.fonts['step_detail'],
            bg=self.COLORS['white'],
            fg=self.COLORS['gray_500']
        ).pack(anchor='w')
    ...
    def create_excel_at_location(self, output_file):
        ...
        for q_idx, q in enumerate(questions):
            ...
            for idx, response in enumerate(response_options):
                ...
                # Calculate percentage within each column
                col_idx = 2
                for grp_idx, group_val in enumerate(breakout_values):
                    ...
                    group_total = len(group_df[group_df[data_col].notna()])
                    
                    if group_total > 0:
                        count = len(self.filter_by_option(group_df, data_col, response, idx, response_code_mapping))
                        pct = count / group_total
                        cell = ws.cell(row=row_num, column=col_idx, value=pct)
                        cell.number_format = '0%'
                    else:
                        cell = ws.cell(row=row_num, column=col_idx, value=0)
                        cell.number_format = '0%'
                ...
                # Total column
                total_respondents = len(df[df[data_col].notna()])
                if total_respondents > 0:
                    ...
                    cell = ws.cell(row=row_num, column=col_idx, value=total_pct)
                    cell.number_format = '0%'
                else:
                    cell = ws.cell(row=row_num, column=col_idx, value=0)
                    cell.number_format = '0%'
            ...
            grand_total = len(df[df[data_col].notna()])
    ...
    def create_pdf_at_location(self, output_file):
        ...
        for q_idx, q in enumerate(questions):
            ...
            for idx, response in enumerate(response_options):
                ...
                # Calculate percentage within each column (department)
                for grp_idx, group_val in enumerate(breakout_values):
                    # Get all people in this department who answered this question
                    group_df = self.filter_by_option(df, breakout_col, group_val, grp_idx, breakout_code_mapping)
                    group_total = len(group_df[group_df[data_col].notna()])
                    
                    if group_total > 0:
                        # Count how many in this department gave this specific response
                        count = len(self.filter_by_option(group_df, data_col, response, idx, response_code_mapping))
                        pct = count / group_total
                        row.append(f"{pct:.0%}")
                    else:
                ...
                # Total column - percentage of all respondents who gave this answer
                total_respondents = len(df[df[data_col].notna()])
                if total_respondents > 0:
                    total_count = len(self.filter_by_option(df, data_col, response, idx, response_code_mapping))
                    total_pct = total_count / total_respondents
                    row.append(f"{total_pct:.0%}")
                else:
                    row.append("0%")
            ...
            # Create table
            col_widths = [3.5*inch] + [0.8*inch] * (len(breakout_values) + 1)
            
            # Adjust column widths if too many columns
            if len(breakout_values) > 8:
                col_widths = [3.0*inch] + [0.6*inch] * (len(breakout_values) + 1)
    ...
    def filter_by_option(self, df, column, option, option_index=None, data_code_mapping=None):
        ...
        for data_val in data_values:
            ...
            if data_num_match:
                ...
                # Check if the text part matches the survey option (without parentheses)
                if data_text and data_text in option_no_parens:
                    mask = df[column] == data_val
                    if mask.any():
                        return df[mask]
                
                # Also check reverse - survey text in data text
                if data_text and option_no_parens in data_text:
                    mask = df[column] == data_val
                    if mask.any():
                        return df[mask]
        ...
        # === METHOD 7: Substring containment ===
        # Check if data value is contained in survey option
        # This handles cases like "Day Shift" matching "Day shift, 7:00 am to 3:00 pm"
        data_values = df[column].dropna().unique()
        for data_val in data_values:
            ...
            if len(data_val_lower) > 10 and len(option_str) > 10:
    ...
    def get_data_code_mapping(self, df, column, num_options):
        ...
        # Non-standard letters (like e, r, w) - map by sorted position
        # This assumes the codes appear in the same order as survey options
        return sorted(data_values, key=lambda x: str(x).lower())
