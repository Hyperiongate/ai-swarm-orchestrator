...
Last Updated: February 19, 2026 - FIXED post-schedule context lock and keyword over-matching

CHANGES February 19, 2026:
- FIXED: After schedule is generated, conversation_context was never cleared from the
  database. Every subsequent message re-entered the schedule flow because in_schedule_flow
  remained True. Fixed by returning an empty context dict on generate_schedule action,
  which causes orchestration_handler.py to overwrite the DB record with a clean state.
- FIXED: detect_schedule_request() was matching phrases like 'shift schedule' in general
  questions (e.g. 'mistakes when implementing a new shift schedule'), hijacking them into
  the schedule flow. Tightened keyword list to require explicit creation/generation intent.
- No other logic changed.

PREVIOUS CHANGES:
- January 27, 2026: FIXED INFINITE LOOP BUG
  * Removed shift start time questions
  * Uses defaults from PatternScheduleGenerator (6am/6pm for 12hr, 7am/3pm/11pm for 8hr)
...
class CombinedScheduleHandler:
    ...
    def detect_schedule_request(self, user_message):
        """
        Detect if user is explicitly requesting a NEW schedule to be created.

        TIGHTENED February 19, 2026: Previously matched broad phrases like
        'shift schedule' or 'schedule pattern' which hijacked general questions
        about scheduling into the schedule generation flow. Now requires explicit
        creation/generation intent words combined with schedule context.
        """
        message_lower = user_message.lower()

        # Must have explicit creation intent
        creation_intent = [
            'create a schedule', 'generate a schedule', 'make a schedule',
            'show me a schedule', 'build a schedule', 'design a schedule',
            'create a shift', 'generate a shift', 'make a shift pattern',
            'help me with scheduling', 'need a schedule', 'want a schedule',
            'i need a schedule', 'i want a schedule', 'can you make a schedule',
            'can you create a schedule', 'can you generate a schedule',
            'build me a schedule', 'create me a schedule'
        ]

        return any(phrase in message_lower for phrase in creation_intent)
...
Which sounds better?"""
    ...
    def format_schedule_response(self, shift_length, pattern_key, filepath):
        ...
        response += "\nYour Excel file is ready to download above. Would you like to generate another schedule or ask any questions?"
        return response
    ...
    def detect_answer(self, user_message, question_type):
        ...
        if question_type == 'coverage':
            ...
            if '4-4' in message_lower:
                return '4-4'
            # Number selection - only when recommendations are shown
            if message_lower.strip() == '1':
                return '2-2-3'
            if message_lower.strip() == '2':
                return 'dupont'
    ...
    def process_request(self, user_message, conversation_context=None):
        ...
        - On generate_schedule, returns empty context dict so orchestration_handler.py
          overwrites the DB with a clean state, fully exiting the schedule flow.
        - Previously returned no context on generate_schedule, leaving in_schedule_flow=True
          in the DB, locking all subsequent messages into the schedule flow.
        ...
        # Check if this is a schedule request (only if NOT already in flow)
        ...
        if not conversation_context.get('weekend_pref'):
            ...
            if weekend_pref:
                ...
                return {
                    'action': 'ask_question',
                    'message': "Please choose **alternating**, **fixed**, or **don't care**.",
                    'context': conversation_context
                }
        ...
        if not conversation_context.get('work_stretch'):
            ...
            if work_stretch:
                ...
                return {
                    'action': 'ask_question',
                    'message': "Please choose **long**, **short**, or **mixed**.",
                    'context': conversation_context
                }

        # 6. Pattern Selection - generate schedule and CLEAR context completely
        if conversation_context.get('recommendations') and not conversation_context.get('selected_pattern'):
            ...
            if pattern_key:
                ...
                # CRITICAL FIX February 19, 2026:
                # Return empty context dict so orchestration_handler.py saves a clean
                # state to the DB, fully exiting the schedule flow for this conversation.
                # Previously no context was returned here, leaving in_schedule_flow=True
                # in the DB and locking all subsequent messages into the schedule flow.
                return {
                    ...
                    'filepath': filepath,
                    'context': {}  # Empty context clears the schedule flow from DB
                }
            else:
                return {
                    'action': 'show_recommendations',
                    'message': "I didn't recognize that. " + self.format_recommendations(
                        conversation_context['recommendations'], conversation_context
                    ),
                    'context': conversation_context
                }

        # Fallback - reset to clean state
        return {
            'action': 'ask_question',
            'message': self.build_coverage_question(),
            'context': {}
