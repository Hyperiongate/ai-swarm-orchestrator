...
# Match config.py DATABASE setting
DATABASE = os.environ.get('DATABASE_URL', 'swarm_intelligence.db')

# If DATABASE_URL is a postgres URL (Render), fall back to local sqlite for migration
# (This migration targets SQLite only)
if DATABASE.startswith('postgresql') or DATABASE.startswith('postgres'):
    # Try local sqlite path
