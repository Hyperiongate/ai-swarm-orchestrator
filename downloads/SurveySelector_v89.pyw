...
Version 89.0 - ADDED TWO-LEVEL BREAKOUT ANALYSIS
Last Updated: February 20, 2026

NEW IN V89 (February 20, 2026):
- ✅ Two-level breakout analysis: e.g. "By Department by Length of Service"
- ✅ After selecting primary breakouts in Step 2, optional secondary breakout
      dialog allows choosing a second segmentation variable for each primary
- ✅ PDF and Excel reports expand columns: DeptA/0-5yr, DeptA/6-10yr, etc.
- ✅ Column widths auto-adjust based on number of combined columns
- ✅ Secondary breakout stored in project file (survives reopen)
- ✅ Fully backward compatible - existing single-level breakouts unchanged
- ✅ "Skip" option in secondary dialog preserves original behavior
...
class SurveyCrosstabApp:
    ...
    def __init__(self):
        ...
        self.secondary_breakouts = {}  # Dict: primary_q_num -> secondary_q_num (or None)
    ...
    def run(self):
        ...
        self.root = tk.Tk()
        self.root.title("Survey Crosstab Tool - With Two-Level Breakout & Insights Report")
    ...
    def create_footer(self):
        ...
        version = tk.Label(
            footer,
            text="Survey Crosstab Tool v89.0 - Now with Two-Level Breakout Analysis",
            font=self.fonts['small'],
            bg=self.COLORS['white'],
            fg=self.COLORS['gray_500']
        )
    ...
    def start_new_project_from_step4(self):
        """Reset all state and return to Step 1 so the user can start fresh.
        Added v89.1 - February 20, 2026: Gives users a way to exit Step 4.
        """
        ...
        self.questions = []
        self.questions_with_options = []
        self.selected_breakouts = []
        self.selected_breakout = None
        self.secondary_breakouts = {}
        self.remark_file = None
        self.remark_df = None
    ...
    def open_project(self, project_name):
        ...
        def process():
            try:
                ...
                # Load secondary breakouts (v89+)
                self.secondary_breakouts = self.project_data.get('secondary_breakouts', {})
    ...
    def confirm_breakout(self):
        ...
        # v89: Offer optional secondary breakout selection
        self.show_secondary_breakout_dialog()
    
    def show_secondary_breakout_dialog(self):
        """
        v89: Optional secondary breakout dialog.
        
        For each selected primary breakout, lets the user optionally pick a
        second question to cross-tabulate against.  Example: primary = Department,
        secondary = Length of Service  ->  columns become Dept/Service combos.
        
        The user can skip this entirely (click 'Skip - No Secondary Breakout')
        and the tool behaves exactly as it did in v88.
        
        Added: February 20, 2026
        """
        dialog = tk.Toplevel(self.root)
        dialog.title("Optional: Secondary Breakout Variables")
        ...
        tk.Label(
            title_frame,
            text="Optional: Add Secondary Breakout",
            font=self.fonts['step_title'],
            bg=self.COLORS['gray_50'],
            fg=self.COLORS['gray_900']
        ).pack(anchor='w')
        ...
        tk.Label(
            title_frame,
            text="For each primary breakout, optionally choose a second segmentation.\n"
                 "Example: By Department  →  By Department by Length of Service\n"
                 "Leave a dropdown at '(None)' to keep that breakout as single-level.",
            font=self.fonts['small'],
            bg=self.COLORS['gray_50'],
            fg=self.COLORS['gray_600'],
            justify=tk.LEFT,
        ...
        # Scrollable area for each primary breakout
        scroll_frame_outer = tk.Frame(dialog, bg=self.COLORS['white'])
        scroll_frame_outer.pack(fill=tk.BOTH, expand=True, padx=20, pady=10)
        
        canvas = tk.Canvas(scroll_frame_outer, bg=self.COLORS['white'], highlightthickness=0)
        ...
        # Build question display list for dropdowns (all questions except the primary itself)
        questions = self.questions_with_options or self.questions
        
        def get_q_display(q_num):
            """Return display label for a question number"""
            for q in questions:
                if str(q['number']) == str(q_num):
                    text = q.get('text', f'Question {q_num}')
                    return f"Q{q_num}: {text[:55]}{'...' if len(text) > 55 else ''}"
            return f"Q{q_num}"
        
        # All available secondary choices (full list, will exclude primary per row)
        all_q_labels = [(str(q['number']), get_q_display(q['number'])) for q in questions]
        
        # Dict to hold the StringVar for each primary breakout row
        secondary_vars = {}
        
        for primary_num in self.selected_breakouts:
            primary_label = get_q_display(primary_num)
            ...
            inner = tk.Frame(row_frame, bg=self.COLORS['white'])
            inner.pack(fill=tk.X, padx=12, pady=10)
            ...
            tk.Label(
                inner,
                text="Secondary breakout (optional):",
                font=self.fonts['small'],
                bg=self.COLORS['white'],
                fg=self.COLORS['gray_600']
            ).pack(anchor='w', pady=(6, 2))
            
            # Build dropdown options excluding the primary question itself
            dropdown_options = ['(None - single level only)'] + [
                label for num, label in all_q_labels
                if str(num) != str(primary_num)
            ]
            # Map display label back to question number
            label_to_num = {label: num for num, label in all_q_labels}
            
            # Pre-select existing secondary if already set
            existing_sec = self.secondary_breakouts.get(str(primary_num))
            if existing_sec:
                existing_label = get_q_display(existing_sec)
                initial = existing_label if existing_label in dropdown_options else dropdown_options[0]
            else:
                initial = dropdown_options[0]
            
            var = tk.StringVar(value=initial)
            secondary_vars[str(primary_num)] = (var, label_to_num)
            ...
            dropdown.pack(anchor='w', fill=tk.X)
        
        # Buttons
        button_frame = tk.Frame(dialog, bg=self.COLORS['white'])
        button_frame.pack(fill=tk.X, padx=20, pady=(5, 20))
        
        def _advance_after_secondary():
            """
            Go to Step 4 if Remark data is already loaded, otherwise Step 3.
            This handles both the new-project flow (no Remark yet -> Step 3)
            and the edit-breakout flow (Remark already loaded -> Step 4).
            """
            dialog.destroy()
            if self.remark_file and os.path.exists(self.remark_file):
                self.current_step = 4
            else:
                self.current_step = 3
            self.show_current_step()
        ...
        def on_skip():
            """Skip secondary breakout entirely - clear any previous selections"""
            self.secondary_breakouts = {}
            self.project_data['secondary_breakouts'] = {}
            self.save_project()
            _advance_after_secondary()
        
        def on_continue():
            """Save secondary breakout selections and proceed"""
            new_secondary = {}
            for primary_num, (var, label_to_num) in secondary_vars.items():
                selected_label = var.get()
                if selected_label and selected_label != '(None - single level only)':
                    sec_num = label_to_num.get(selected_label)
                    if sec_num:
                        new_secondary[str(primary_num)] = str(sec_num)
            
            self.secondary_breakouts = new_secondary
            self.project_data['secondary_breakouts'] = new_secondary
            self.save_project()
            _advance_after_secondary()
        
        tk.Button(
            button_frame,
            text="Skip - No Secondary Breakout",
            font=self.fonts['button'],
            bg=self.COLORS['gray_200'],
            fg=self.COLORS['gray_700'],
            relief=tk.FLAT,
    ...
    def show_step_4(self):
        ...
        # ── v89.1: Start New Project button ───────────────────────────────────
        ...
        new_project_btn = tk.Button(
            ...
            pady=12,
            cursor='hand2',
            command=self.start_new_project_from_step4
        )
        new_project_btn.pack(fill=tk.X, pady=(0, 30))
        # ── end v89.1 ──────────────────────────────────────────────────────────
    ...
    def create_excel_at_location(self, output_file):
        ...
        v89: Supports two-level breakout when self.secondary_breakouts has an
        entry for the current self.selected_breakout.  Columns expand to
        primary x secondary combinations.  Single-level behavior (v88) is
        fully preserved when no secondary breakout is configured.
        
        Added two-level support: February 20, 2026
        """
        ...
        # Find the primary breakout column in data
        ...
        # ── v89: Check for secondary breakout ──────────────────────────────────
        secondary_q_num = self.secondary_breakouts.get(str(self.selected_breakout))
        secondary_col = None
        secondary_values = []
        ...
        if secondary_q_num:
            secondary_col = self.find_data_column(df, secondary_q_num)
            if secondary_col:
                for q in questions:
                    if str(q['number']) == str(secondary_q_num):
                        secondary_question = q
                        break
                if secondary_question and secondary_question.get('options'):
                    secondary_values = secondary_question['options']
                else:
                    secondary_values = sorted(df[secondary_col].dropna().unique().tolist())
                secondary_code_mapping = self.get_data_code_mapping(df, secondary_col, len(secondary_values))
                secondary_text = secondary_question.get('text', f'Q{secondary_q_num}')[:30] if secondary_question else f'Q{secondary_q_num}'
            else:
                # Secondary column not found - fall back to single-level
                secondary_q_num = None
        # ── end v89 secondary setup ────────────────────────────────────────────
        ...
        row_num = 1
        if secondary_q_num and secondary_values:
            header_label = f"{breakout_text[:25]} / {secondary_text[:20]}"
            col_headers = [header_label]
            for pv in breakout_values:
                for sv in secondary_values:
                    col_headers.append(f"{str(pv)[:10]}/{str(sv)[:10]}")
            col_headers.append('Total')
        else:
            col_headers = [breakout_text] + [str(v)[:20] for v in breakout_values] + ['Total']
        
        total_excel_cols = len(col_headers)
        ...
        # Process each question (excluding the breakout and secondary questions)
        ...
        for q_idx, q in enumerate(questions):
            ...
            if str(q_num) == str(self.selected_breakout):
                continue
            if secondary_q_num and str(q_num) == str(secondary_q_num):
                continue
            ...
            cell.border = thin_border
            ws.merge_cells(start_row=row_num, start_column=1, end_row=row_num, end_column=total_excel_cols)
            row_num += 1
            ...
            if secondary_q_num and secondary_values:
                # Two-level: primary x secondary
                for grp_idx, group_val in enumerate(breakout_values):
                    primary_df = self.filter_by_option(df, breakout_col, group_val, grp_idx, breakout_code_mapping)
                    for sec_idx, sec_val in enumerate(secondary_values):
                        group_df = self.filter_by_option(primary_df, secondary_col, sec_val, sec_idx, secondary_code_mapping)
                        ...
                        if group_total > 0:
                            col_pcts = [
                                len(self.filter_by_option(group_df, data_col, response, idx, response_code_mapping)) / group_total
                                for idx, response in enumerate(response_options)
                            ]
                        else:
                            col_pcts = [0.0] * len(response_options)
                        column_percentages.append(col_pcts)
            else:
                # Single-level (original v88 logic - unchanged)
                for grp_idx, group_val in enumerate(breakout_values):
            ...
            # Total column (always uses full dataset)
            valid_total = df[df[data_col].notna()]
            valid_total = valid_total[valid_total[data_col] != 'BLANK']
            total_respondents = len(valid_total)
            if total_respondents > 0:
                total_col_pcts = [
                    len(self.filter_by_option(df, data_col, response, idx, response_code_mapping)) / total_respondents
                    for idx, response in enumerate(response_options)
                ]
            ...
            adjusted_columns = [self.adjust_percentages_to_100(col_pcts) for col_pcts in column_percentages]
            ...
            # Grand total (always from full dataset)
        ...
        data_col_count = total_excel_cols - 1  # exclude label column
        col_width = max(8, min(14, int(100 / data_col_count))) if data_col_count > 0 else 12
        for col_idx in range(2, total_excel_cols + 1):
    ...
    def create_pdf_at_location(self, output_file):
        ...
        v89.2 - February 20, 2026
        Two-level breakout redesign: instead of exploding columns horizontally,
        the report now generates ONE SECTION PER PRIMARY GROUP, each starting on
        a new page.  Within each section the secondary breakout becomes the
        columns (e.g. Yes | No | Group Total).  A final summary section at the
        end shows the standard single-level primary breakout across all
        respondents.

        Single-level behavior (no secondary breakout configured) is identical
        to v88 and completely unchanged.
        """
        from reportlab.lib import colors
        from reportlab.lib.pagesizes import letter, landscape
        from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
        ...
        breakout_code_mapping = self.get_data_code_mapping(df, breakout_col, len(breakout_values))
        breakout_text = (breakout_question.get('text', f'Question {self.selected_breakout}')[:60]
                         if breakout_question else f'Q{self.selected_breakout}')

        # ── Secondary breakout setup ───────────────────────────────────────────
        secondary_q_num = self.secondary_breakouts.get(str(self.selected_breakout))
        secondary_col = None
        secondary_values = []
        secondary_code_mapping = []
        ...
        if secondary_q_num:
            ...
            if secondary_col:
                ...
                if secondary_question and secondary_question.get('options'):
                    ...
                    secondary_values = sorted(df[secondary_col].dropna().unique().tolist())
                secondary_code_mapping = self.get_data_code_mapping(
                    df, secondary_col, len(secondary_values))
                secondary_text = (secondary_question.get('text', f'Q{secondary_q_num}')[:50]
                                  if secondary_question else f'Q{secondary_q_num}')
            else:
                secondary_q_num = None   # column not in data - fall back to single-level

        # ── Helper: build one crosstab table ──────────────────────────────────
        def build_question_table(q, col_headers, group_df_list, total_df):
            """
            Build a single crosstab Table for question q.

            col_headers  : list of header strings (label col + data cols + 'Total')
            group_df_list: list of (label, dataframe) for each data column
                           (the dataframe is pre-filtered to that group)
            total_df     : dataframe to use for the Total column
            """
            q_num  = q['number']
            q_text = q.get('text', f'Question {q_num}')
            data_col = self.find_data_column(df, q_num)
            ...
            response_code_mapping = self.get_data_code_mapping(
                df, data_col, len(response_options))

            is_last = (q.get('_idx', 0) == total_questions - 1)
            special_calc = self.get_special_calculation(
                q_num, q_text, response_options, is_last)

            # ── percentages ────────────────────────────────────────────────────
            all_pcts = []
            for _label, gdf in group_df_list:
                valid = gdf[gdf[data_col].notna()]
                valid = valid[valid[data_col] != 'BLANK']
                gtotal = len(valid)
                if gtotal > 0:
                    pcts = [
                        len(self.filter_by_option(gdf, data_col, r, i, response_code_mapping)) / gtotal
                        for i, r in enumerate(response_options)
                    ]
                else:
                    pcts = [0.0] * len(response_options)
                all_pcts.append(pcts)

            # Total column
            valid_tot = total_df[total_df[data_col].notna()]
            valid_tot = valid_tot[valid_tot[data_col] != 'BLANK']
            tot_n = len(valid_tot)
            if tot_n > 0:
                tot_pcts = [
                    len(self.filter_by_option(total_df, data_col, r, i, response_code_mapping)) / tot_n
                    for i, r in enumerate(response_options)
                ]
            else:
                tot_pcts = [0.0] * len(response_options)
            all_pcts.append(tot_pcts)

            adjusted = [self.adjust_percentages_to_100(col) for col in all_pcts]

            # ── assemble rows ──────────────────────────────────────────────────
            table_data = [col_headers]
            q_header   = [f"{q_num}  {q_text[:80]}"] + [''] * (len(col_headers) - 1)
            table_data.append(q_header)

            for idx, response in enumerate(response_options):
                row = [f"    {str(response)[:50]}"]
                for col_adj in adjusted:
                    row.append(f"{col_adj[idx]}%")
                table_data.append(row)

            # Total (n) row
            total_row = ["    Total"]
            for _label, gdf in group_df_list:
                n = len(gdf[gdf[data_col].notna()])
                total_row.append(str(n))
            total_row.append(str(len(total_df[total_df[data_col].notna()])))
            table_data.append(total_row)
            ...
            if special_calc:
                ...
                avg_row = [f"    {avg_label}"]
                for _label, gdf in group_df_list:
                    avg = self.calculate_special_average(
                        gdf, data_col, response_options, special_calc)
                    avg_row.append(f"{avg:.2f}" if avg is not None else "N/A")
                tot_avg = self.calculate_special_average(
                    total_df, data_col, response_options, special_calc)
                avg_row.append(f"{tot_avg:.2f}" if tot_avg is not None else "N/A")
                table_data.append(avg_row)

            # ── column widths ──────────────────────────────────────────────────
            num_data_cols = len(col_headers) - 1
            usable = 10.0
            if num_data_cols <= 8:
                label_w = 2.8
                data_w  = max(0.42, min((usable - label_w) / num_data_cols, 0.80))
            elif num_data_cols <= 16:
                label_w = 2.4
                data_w  = max(0.38, min((usable - label_w) / num_data_cols, 0.60))
            else:
                label_w = 2.0
                data_w  = max(0.32, (usable - label_w) / num_data_cols)
            col_widths = [label_w * inch] + [data_w * inch] * num_data_cols
            ...
            if special_calc:
                ...
                style_cmds.extend([
                    ('FONTNAME',   (0, avg_row_idx), (-1, avg_row_idx),
                                   'Helvetica-BoldOblique'),
                    ('BACKGROUND', (0, avg_row_idx), (-1, avg_row_idx),
            ...
            return tbl
        # ── end build_question_table ───────────────────────────────────────────

        # ── Questions to analyse (skip primary and secondary breakout Qs) ──────
        analysis_questions = []
        for q_idx, q in enumerate(questions):
            q_num = q['number']
            if str(q_num) == str(self.selected_breakout):
            ...
            if secondary_q_num and str(q_num) == str(secondary_q_num):
                continue
            q['_idx'] = q_idx   # stash original index for is_last logic
            analysis_questions.append(q)

        # ══════════════════════════════════════════════════════════════════════
        # TWO-LEVEL PATH: one page-section per primary group + summary section
        # ══════════════════════════════════════════════════════════════════════
        if secondary_q_num and secondary_values:

            sec_col_headers = [f'Q{self.selected_breakout} group'] + \
                              [str(sv)[:18] for sv in secondary_values] + \
                              ['Group Total']

            for grp_idx, group_val in enumerate(breakout_values):
                # Filter df to this primary group
                primary_df = self.filter_by_option(
                    df, breakout_col, group_val, grp_idx, breakout_code_mapping)

                # Section page-break (not before very first section)
                if grp_idx > 0:
                ...
                elements.append(Paragraph(
                    f"<b>Q{self.selected_breakout}: {breakout_text}</b>  —  "
                    f"Group: <b>{str(group_val)}</b>  "
                    f"(n = {n_primary})",
                    styles['SectionTitle']
                ))
                elements.append(Paragraph(
                    f"Secondary breakout: Q{secondary_q_num}: {secondary_text}",
                    styles['SectionSubtitle']
                ))
                elements.append(Spacer(1, 0.08*inch))

                # Build group_df_list for this primary group
                group_df_list = [
                    (str(sv), self.filter_by_option(
                        primary_df, secondary_col, sv, si, secondary_code_mapping))
                    for si, sv in enumerate(secondary_values)
                ]

                # One table per analysis question
                for q in analysis_questions:
                    tbl = build_question_table(
                        q,
                        col_headers   = sec_col_headers,
                        group_df_list = group_df_list,
                        total_df      = primary_df,   # Total = this primary group
                    )
                    if tbl is not None:
                        elements.append(tbl)
                        elements.append(Spacer(1, 0.15*inch))

            # ── Summary section: primary breakout only (all respondents) ───────
            elements.append(PageBreak())
            elements.append(Paragraph(
                f"<b>Summary — Q{self.selected_breakout}: {breakout_text}</b>  "
                f"(all respondents)",
                styles['SectionTitle']
            ))
            elements.append(Paragraph(
                "Overall results by primary breakout group, no secondary filter applied.",
                styles['SectionSubtitle']
            ))
            elements.append(Spacer(1, 0.08*inch))

            summary_col_headers = [breakout_text[:40]] + \
                                  [str(v)[:12] for v in breakout_values] + \
                                  ['Total']
            summary_group_list  = [
                (str(gv), self.filter_by_option(
                    df, breakout_col, gv, gi, breakout_code_mapping))
                for gi, gv in enumerate(breakout_values)
            ]

            for q in analysis_questions:
                tbl = build_question_table(
                    q,
                    col_headers   = summary_col_headers,
                    group_df_list = summary_group_list,
                    total_df      = df,
                )
                if tbl is not None:
                    elements.append(tbl)
        ...
        # ══════════════════════════════════════════════════════════════════════
        # SINGLE-LEVEL PATH: identical to v88
        # ══════════════════════════════════════════════════════════════════════
        else:
            col_headers = ([breakout_text] +
                           [str(v)[:12] for v in breakout_values] +
                           ['Total'])
            group_df_list = [
                (str(gv), self.filter_by_option(
